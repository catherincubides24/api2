export default function SectionTitle({ eyebrow, title, description, align = "left" }) {
  ...
  return (
    <div className={["space-y-2", alignClass].join(" ")}>
      ...
      <h2 className="font-display text-3xl leading-tight text-ink dark:text-cream sm:text-4xl">
        {title}
      </h2>
      {description && (
        <p className="max-w-2xl text-ink/70 dark:text-cream/70">{description}</p>
      )}
    </div>
  );
}